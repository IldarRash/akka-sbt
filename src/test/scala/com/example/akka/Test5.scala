package com.example.akka

import akka.actor.{ActorSystem, Props}
import akka.testkit.TestKit
import com.example.akka.crashes.LifeCycle
import com.example.akka.crashes.LifeCycle.{ForceRestart, SampleMessage}
import org.scalatest.{BeforeAndAfterAll, WordSpecLike}

class Test5 extends TestKit(ActorSystem("LifCycleTest"))
  with WordSpecLike with BeforeAndAfterAll {

  override def afterAll(): Unit = {
    system.terminate()
  }

  "The Child" must {
    "log lifecycle hooks" in {
        val testActorRef = system.actorOf(
          Props[LifeCycle], "LifeCycleHooks")
        watch(testActorRef)
        testActorRef ! ForceRestart
        testActorRef.tell(SampleMessage, testActor)
        expectMsg(SampleMessage)
        system.stop(testActorRef)
        expectTerminated(testActorRef)
      }
    }

}
